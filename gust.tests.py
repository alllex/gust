...
Offline, against a local git fixture repository and fake Gradle launchers.
...
# A stand-in for Gradle. On `NAME wrapper --gradle-version V ...`, gradlew becomes a copy of it, but only
# inside a Gradle build (with a settings file), as with Gradle's wrapper task. On `--version`, a Gradle
# banner is printed. Anything else is echoed.
FAKE_GRADLE = """#!/bin/sh
if [ "$1" = "--gradle-user-home" ]; then shift 2; fi   # first on every invocation
if [ "$1" = "--version" ]; then printf '\\nWelcome to Gradle 9.7.1!\\n\\n------------\\nGradle 9.7.1\\n------------\\n'; exit 0; fi
if [ "$1" = "wrapper" ]; then
  if [ ! -f settings.gradle ] && [ ! -f settings.gradle.kts ] && [ ! -f settings.gradle.dcl ]; then
    echo "Directory '$PWD' does not contain a Gradle build."; exit 1
  fi
  cp "$0" gradlew; chmod 755 gradlew
  echo "installed wrapper $3 via NAME"
  exit 0
fi
echo "$(basename "$0")" "$@"
"""
...
GECHO = """#!/bin/sh
if [ "$1" = "--gradle-user-home" ] && [ "$3" = "--version" ]; then printf 'Gradle 9.7.1\\n'; exit 0; fi
echo "$@"
"""
...
class GustCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.bin.mkdir()
        self.script("gecho", GECHO)
        os.environ["PATH"] = f"{self.bin}:{self._env['PATH']}"
    ...
    def fake_gradle_on_path(self, name="gradle"):
        return self.script(name, FAKE_GRADLE.replace("NAME", name))
...
""")
    ...
    def test_readme_quick_start_scenario_parses(self):
        readme = Path(gs.__file__).with_name("README.md").read_text()
...
class LayoutRemoteTest(GustCase):
    ...
    def setUpClass(cls):
        ...
        (cls.repo / "gradlew").write_text(FAKE_GRADLE.replace("NAME", "gradlew"))
        (cls.repo / "gradlew").chmod(0o755)
        (cls.repo / "sub").mkdir()
    ...
    def test_fetch_copy_reuse_and_layering(self):
        layout = f'setup.layout.remote = "{self.repo}#{self.sha}"\n[setup.layout.project]\n"sub/note.txt" = "own"\n"extra.txt" = "x"'
        steps = '[[steps]]\nedit = [{ file = "settings.gradle.kts", replace = "remote", with = "edited" }]\n[[steps]]\nrun.shell = "test -x gradlew"\n'
        ...
        self.assertEqual(summary.remote, {"link": f"{self.repo}#{self.sha}", "checkout": str(checkout), "fetched": True})
        printed = self.out.getvalue()
        self.assertTrue(printed.startswith(f"remote:   {self.repo}#{self.sha}\ncheckout: {checkout}\n          fetching {self.sha} from {self.repo}\nscenario: s\n"))
        ...
        self.assertTrue(os.access(project / "gradlew", os.X_OK))                        # mode preserved
    ...
    def test_subdir_and_relative_gradle(self):
        scenario = self.load(f'setup.layout.remote = "{self.repo}#{self.sha}/sub"', '[[steps]]\nrun.shell = "test -f build.gradle.kts && test ! -e sub"\n')
        ...
        scenario = self.load(f'setup.layout.remote = "{self.repo}#{self.sha}"', '[[steps]]\nrun.gradle = { args = "help", expect = { output = ["gradlew help"] } }\n')
        summary = self.run_scenario(scenario, self.tmp / "t.out", gradle="./gradlew", stop_daemons=False)
        self.assertEqual(summary.status, "ok")
        self.assertIn("gradle:   ./gradlew 9.7.1 (--gradle)", self.out.getvalue())
    ...
    def test_checks_after_resolve(self):
        ...
        scenario = self.load(f'setup.layout.remote = "{self.repo}#{self.sha}"\n[setup.layout.project]\n"sub/build.gradle.kts/x" = "c"')
        with self.assertRaises(gs.GustError) as cm:
            self.run_scenario(scenario, self.tmp / "s.out", gradle="gecho", stop_daemons=False)
        self.assertIn("conflicts with 'sub/build.gradle.kts' (setup.layout.remote)", str(cm.exception))
        scenario = self.load(f'setup.layout.remote = "{self.repo}#{self.sha}/missing"')
        with self.assertRaises(gs.GustError) as cm:
            self.run_scenario(scenario, self.tmp / "s.out", gradle="gecho", stop_daemons=False)
        self.assertIn("'missing' is not a directory", str(cm.exception))
        scenario = self.load(f'setup.layout.remote = "{self.repo}#{"f" * 40}"')
        with self.assertRaises(gs.GustError) as cm:
            self.run_scenario(scenario, self.tmp / "s.out", gradle="gecho", stop_daemons=False)
    ...
    def test_mismatched_checkout_refused(self):
        ...
        short = self.load(f'setup.layout.remote = "{self.repo}#{self.sha[:12]}"')
    ...
    def test_gradle_wrapper_field_without_gradlew(self):
        scenario = self.load(f'setup.gradle = "wrapper"\nsetup.layout.remote = "{self.repo}#{self.sha}/sub"', '[[steps]]\nrun.gradle = "help"\n')
        with self.assertRaises(gs.GustError) as cm:
            self.run_scenario(scenario, self.tmp / "s.out", stop_daemons=False)
        self.assertIn('setup.gradle = "wrapper" but gradlew is not in the layout or the remote checkout', str(cm.exception))
        self.assertFalse((self.tmp / "s.out").exists())
        inline = gs.load_scenario('name = "i"\nsetup.gradle = "wrapper"\n[setup.layout.project]\n"gradlew" = "#!/bin/sh"\n[[steps]]\nrun.gradle = "help"')
        with self.assertRaises(gs.GustError) as cm:
    ...
    def test_version_installs_wrapper_via_gradle_on_path(self):
        ...
        self.assertTrue(os.access(out_dir / "project" / "gradlew", os.X_OK))
        ...
        self.assertRegex(printed, r"gradle:   \./gradlew 9\.7\.1 \(scenario, installed by gradle\)\n"
        ...
        self.assertIn("gradle:   ./gradlew 9.8.0-rc-2 (--gradle, installed by gradle)\n[WRP]", self.out.getvalue())
    ...
    def test_version_precedence_and_rejections(self):
        ...
        self.assertIn("gradle:   ./gradlew 9.7.0 (--gradle, installed by gradle)", self.out.getvalue())
        with self.assertRaises(gs.GustError) as cm:
    ...
    def test_gradle_value_resolution(self):
        ...
        refused("/no/such/gradle", "--gradle '/no/such/gradle': no executable at /no/such/gradle")
        ...
        summary = self.run_scenario(scenario, self.tmp / "a.out", gradle="bin/gradle", stop_daemons=False)
        ...
        summary = self.run_scenario(scenario, self.tmp / "h.out", gradle="~/bin/gradle", stop_daemons=False)
        ...
        self.script("mygradle", FAKE_GRADLE.replace("NAME", "mygradle"), where=self.tmp)
        summary = self.run_scenario(scenario, self.tmp / "c.out", gradle="mygradle", stop_daemons=False)
        self.assertEqual(summary.gradle, str(self.tmp / "mygradle"))
        ...
        refused("echo", "--gradle 'echo': not a Gradle binary (no 'Gradle <version>' line in the output of --version)")
        self.script("broken", "#!/bin/sh\necho first >&2; echo boom >&2; exit 5\n", where=self.tmp)
        refused("broken", f"--gradle 'broken': cannot run '{self.tmp / 'broken'} --version' (first; boom)")
        (self.bin / "gradle").write_text("#!/bin/sh\necho no version here\n")
        with self.assertRaises(gs.GustError) as cm:
    ...
    def test_version_install_failure(self):
        gradle = self.fake_gradle_on_path()
        gradle.write_text("#!/bin/sh\nif [ \"$3\" = --version ]; then echo 'Gradle 9.7.1'; exit 0; fi\necho boom; exit 7\n")
    ...
    def test_cli_version_values(self):
        ...
        self.assertRegex(out, r"scenario: s\nout:      .*\nhome:     .*\ngradle:   \./gradlew 9\.7\.0 \(--gradle, installed by gradle\)\n"
                              r"\[WRP\] Install Gradle wrapper 9\.7\.0\n\$ gradle wrapper .*\nok: exit 0 in .* -> install-wrapper\.log\nresult: set up\nout:      .*\n")
        shutil.rmtree(self.tmp / "s.out")                                             # without a project: the hint comes first
    ...
    def test_scenario_version_is_probed_once_set_up(self):
        ...
        code, out, _ = self.cli("setup", str(s))
        self.assertEqual(code, gs.EXIT_OK, out)
        self.assertIn("gradle:   ./gradlew 9.7.0 (scenario, installed by gradle)\n", out)      # setup: the requested version, as given
    ...
    def test_wrapper_in_the_project_is_the_default(self):
        ...
        self.assertEqual((summary.status, summary.gradle, summary.gradle_version), ("ok", "./gradlew", "9.7.1"))
        self.assertIn("gradle:   ./gradlew 9.7.1 (default)\n", self.out.getvalue())
        ...
        self.assertEqual(code, gs.EXIT_OK, out)
        for argv, block in ((["step", str(s), "2"], "[2/2] Run: gradle tasks\n$ ./gradlew tasks\n"), (["stop-daemons", str(s)], "[STOP] Stop Gradle daemons\n$ ./gradlew --stop\n")):
            code, out, _ = self.cli(*argv)
            self.assertEqual(code, gs.EXIT_OK, out)
            self.assertIn("gradle:   ./gradlew 9.7.1 (default)\n", out)
            self.assertIn(block, out)
        # check on an uncached remote without setup.gradle: the default is unknown, so noted, not probed
        shutil.rmtree(self.gust / "remotes")
        code, out, _ = self.cli("check", str(self.write(f'setup.layout.remote = "{self.repo}#{self.sha}"', '[[steps]]\nrun.gradle = "help"\n')))
        self.assertEqual(code, gs.EXIT_OK, out)
    ...
    def test_check_command(self):
        ...
        self.assertEqual(code, gs.EXIT_OK, out)
        self.assertEqual(out, f"remote:   {self.repo}#{self.sha} (not cached; fetched on run)\nscenario: s\n          Checks the remote.\n"
                              f"home:     {self.gust / gs.SHARED_HOME}\ngradle:   ./gradlew (scenario; not checked, remote not cached)\n"
        ...
        self.assertEqual(json.loads(out.rstrip("\n").splitlines()[-1]),
                         {"scenario": "s", "description": "Checks the remote.", "gradle": "./gradlew", "gradle_version": "9.7.1",
                          "steps": 3, "step_counts": {"gradle": 1, "shell": 1, "write": 0, "edit": 1},
                          "remote": {"link": f"{self.repo}#{self.sha}", "cached": True}, "status": "ok"})
        self.assertFalse((self.tmp / "s.out").exists())
        ...
        self.assertEqual(code, gs.EXIT_OK)
        self.assertIn("gradle:   ./gradlew 9.7.0 (--gradle, installed by gradle)\n", out)
        self.assertFalse((self.tmp / "s.out").exists())
        ...
        self.assertEqual(code, gs.EXIT_OK)
        self.assertIn("gradle:   gradle 9.7.1 (--gradle)\n", out)
        self.assertEqual(json.loads(out.rstrip("\n").splitlines()[-1])["remote"], {"link": f"{self.repo}#{self.sha}", "cached": False})
        code, out, _ = self.cli("check", str(s), "--gradle", "9.7.0")
        self.assertEqual(code, gs.EXIT_OK)
        self.assertIn("gradle:   ./gradlew 9.7.0 (--gradle, installed by gradle; not checked, remote not cached)\n", out)
        self.assertFalse((self.gust / "remotes").exists())
        code, _, err = self.cli("check", str(s), "--gradle", "echo")
        self.assertEqual(code, gs.EXIT_ERROR)
        self.assertIn("error: --gradle 'echo': not a Gradle binary", err)
...
""")
    ...
    def test_gradle_preconditions_run_before_out_dir_is_touched(self):
        ...
        self.assertIn("no executable at /no/such/gradle", str(cm.exception))
        with self.assertRaises(gs.GustError) as cm:
        ...
        os.environ["PATH"] = str(self.bin)                                            # no gradle here
    ...
    def test_failed_daemon_stop_is_reported(self):
        flaky = self.script("gradle", '#!/bin/sh\nif [ "$3" = "--version" ]; then echo "Gradle 9.7.1"; exit 0; fi\nif [ "$3" = "--stop" ]; then echo cannot; exit 3; fi\necho "$@"\n')
...
class CliTest(GustCase):
    ...
    def test_stop_daemons_command(self):
        ...
        code, _, err = self.cli("stop-daemons", str(s), "--gradle", "false")            # the probe: `false --version` fails
        self.assertEqual(code, gs.EXIT_ERROR)
    ...
    def test_tmp_out_dir(self):
        ...
        self.assertRegex(str(gs.tmp_out("testing-loop")), r"/fake-tmp/gust-\d{6}/testing-loop\.\d{6}\.out$")
        ...
        self.assertRegex(str(out_dir), r"/fake-tmp/gust-\d{6}/s\.\d{6}\.out$")
    ...
    def test_scenario_from_stdin(self):
        ...
        self.assertEqual(code, gs.EXIT_OK)
        self.assertRegex(re.search(r"^out:      (.*)$", out, re.M).group(1), r"/fake-tmp/gust-\d{6}/scenario\.\d{6}\.out$")
...
class StepCommandTest(GustCase):
    ...
    def test_gradle_binary_and_env(self):
        ...
        self.assertEqual(code, gs.EXIT_ERROR)
        self.assertIn("error: --gradle '/opt/g/bin/gradle': no executable at /opt/g/bin/gradle", err)
        custom = self.tmp / "my-home"
    ...
    def test_wrapper_when_the_scenario_sets_gradle(self):
        ...
        code, _, err = self.cli("step", str(self.scenario), "1")
        self.assertEqual(code, gs.EXIT_ERROR)
        self.assertIn("gradlew is not in the layout", err)
        self.script("gradlew", FAKE_GRADLE.replace("NAME", "gradlew"), where=self.project)    # no jar, no properties: nothing is checked
        code, out, _ = self.cli("step", str(self.scenario), "1")
        self.assertEqual(code, gs.EXIT_OK, out)
...
class SpecTest(unittest.TestCase):
    def test_example_is_a_valid_scenario(self):
        ...
        self.assertIn(gs.EXAMPLE, Path(__file__).with_name("README.md").read_text())      # the README quick start, word for word
...
class VersionGateTest(unittest.TestCase):
    def test_gate_precedes_every_version_dependent_import(self):
        source = Path(gs.__file__).read_text().splitlines()
